...
f=`grep -E 'typedef ((const )?[a-z0-9_]+( [a-z0-9_]+)* \*?)t([a-z0-9_]+)\(' $H \
   | sed -r 's/((const )?[a-z0-9_]+( [a-z0-9_]+)* \*?)t([a-z0-9_]+)\(.*/extern t\4 *\4;/i'`
append "$f"
...
echo "$content" | sed -r 's/extern t([a-z0-9_]+).*/t\1 *\1;/gi' >> $DIR/src/udew.c
...
echo "$content" | sed -r 's/extern t([a-z0-9_]+).*/  UDEV_LIBRARY_FIND(\1);/gi' >> $DIR/src/udew.c
